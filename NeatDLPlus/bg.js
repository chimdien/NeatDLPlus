// ============================================================
// Neat DL+ v2.0.0 — Enhanced NeatDownloadManager Extension
// ============================================================

// --- Exclusion Manager ---
var ExclusionManager = (function() {
  var _websites = [];
  var _fileTypes = [];
  var _sizeLimits = {
    minEnabled: false,
  ...
  function init() {
    chrome.storage.sync.get(['excludedWebsites', 'excludedFileTypes', 'sizeLimits'], function(data) {
      _websites = data.excludedWebsites || [];
      _fileTypes = data.excludedFileTypes || [];
      if (data.sizeLimits) _sizeLimits = Object.assign({}, _sizeLimits, data.sizeLimits);
    });
    chrome.storage.onChanged.addListener(function(changes, area) {
      if (area === 'sync') {
        if (changes.excludedWebsites) _websites = changes.excludedWebsites.newValue || [];
        if (changes.excludedFileTypes) _fileTypes = changes.excludedFileTypes.newValue || [];
        if (changes.sizeLimits) _sizeLimits = Object.assign({}, _sizeLimits, changes.sizeLimits.newValue || {});
      }
    });
  }
  ...
  function isWebsiteExcludedSync(url) {
    if (!url || !_websites.length) return false;
    var hostname;
    try { hostname = new URL(url).hostname; } catch(e) { return false; }
    if (!hostname) return false;
    return _websites.some(function(site) {
      var siteHost = site;
      try { siteHost = new URL(site).hostname; } catch(e) { siteHost = site; }
      return hostname === siteHost || hostname.endsWith('.' + siteHost);
    });
  }

  function isFileTypeExcludedSync(filenameOrExt) {
    if (!filenameOrExt || !_fileTypes.length) return false;
    var ext = filenameOrExt;
    if (ext.indexOf('.') >= 0) ext = ext.split('.').pop();
    ext = ext.toLowerCase().trim();
    if (!ext) return false;
    return _fileTypes.some(function(pattern) {
      var patExt = pattern.replace(/^\*\.?/, '').toLowerCase().trim();
      return ext === patExt;
    });
  }

  async function getExcludedWebsites() {
    return new Promise(function(resolve) {
      chrome.storage.sync.get(['excludedWebsites'], function(data) {
        resolve(data.excludedWebsites || []);
      });
    });
  }

  async function addExcludedWebsite(site) {
    var list = await getExcludedWebsites();
    var hostname;
    try { hostname = new URL(site).hostname; } catch(e) { hostname = site; }
    if (hostname && !list.includes(hostname)) {
      list.push(hostname);
      await chrome.storage.sync.set({ excludedWebsites: list });
    }
    return list;
  }

  async function removeExcludedWebsite(site) {
    var list = await getExcludedWebsites();
    list = list.filter(function(s) { return s !== site; });
    await chrome.storage.sync.set({ excludedWebsites: list });
    return list;
  }

  async function getExcludedFileTypes() {
    return new Promise(function(resolve) {
      chrome.storage.sync.get(['excludedFileTypes'], function(data) {
        resolve(data.excludedFileTypes || []);
      });
    });
  }

  async function addExcludedFileType(pattern) {
    var list = await getExcludedFileTypes();
    pattern = pattern.trim().toLowerCase();
    if (!pattern.startsWith('*.')) pattern = '*.' + pattern.replace(/^\*?\.?/, '');
    if (!list.includes(pattern)) {
      list.push(pattern);
      await chrome.storage.sync.set({ excludedFileTypes: list });
    }
    return list;
  }

  async function removeExcludedFileType(pattern) {
    var list = await getExcludedFileTypes();
    list = list.filter(function(p) { return p !== pattern; });
    await chrome.storage.sync.set({ excludedFileTypes: list });
    return list;
  }

  function isFileSizeExcludedSync(sizeBytes) {
    if (!sizeBytes) return false;
    var bytes = typeof sizeBytes === 'number' ? sizeBytes : parseInt(sizeBytes, 10);
    ...
    if (_sizeLimits.minEnabled && typeof _sizeLimits.minValue === 'number' && _sizeLimits.minValue > 0) {
      var minBytes = _sizeLimits.minValue * (multipliers[_sizeLimits.minUnit] || 1048576);
      if (bytes < minBytes) return true;
    }

    if (_sizeLimits.maxEnabled && typeof _sizeLimits.maxValue === 'number' && _sizeLimits.maxValue > 0) {
      var maxBytes = _sizeLimits.maxValue * (multipliers[_sizeLimits.maxUnit] || 1048576);
  ...
  async function getSizeLimits() {
    return new Promise(function(resolve) {
      chrome.storage.sync.get(['sizeLimits'], function(data) {
        resolve(data.sizeLimits || _sizeLimits);
      });
    });
  }

  async function setSizeLimits(limits) {
    _sizeLimits = Object.assign({}, _sizeLimits, limits);
    await chrome.storage.sync.set({ sizeLimits: _sizeLimits });
    return _sizeLimits;
  }
  ...
  return {
    ...
    isWebsiteExcludedSync: isWebsiteExcludedSync,
    isFileTypeExcludedSync: isFileTypeExcludedSync,
    isFileSizeExcludedSync: isFileSizeExcludedSync,
    getExcludedWebsites: getExcludedWebsites,
    addExcludedWebsite: addExcludedWebsite,
    removeExcludedWebsite: removeExcludedWebsite,
    getExcludedFileTypes: getExcludedFileTypes,
    addExcludedFileType: addExcludedFileType,
    removeExcludedFileType: removeExcludedFileType,
    getSizeLimits: getSizeLimits,
    setSizeLimits: setSizeLimits
  };
})();

// --- Original NeatDownloadManager code (modified for Neat DL+) ---
...
function U(){var a=this.constructor.prototype,b;for(b in a)this[b]=a[b].bind(this);this.H={};this.h={};this.m={};this.fa=1;this.u="";this.C=!1;chrome.contextMenus.removeAll();chrome.contextMenus.create({title:"Download with Neat DL+",id:"NDM_CtxMenu",contexts:["link","image"]});chrome.contextMenus.create({title:"Exclude this site from Neat DL+",id:"NeatDLP_ExcludeSite",contexts:["page","frame","link","image"]});this.j(chrome.contextMenus.onClicked,this.W);this.j(chrome.downloads.onCreated,this.X);this.j(chrome.runtime.onConnect,this.Z);this.j(chrome.webRequest.onBeforeRequest,this.T,{urls:["http://*/*","https://*/*","ftp://*/*"],
...
this.M();var c=this;this.F=!0;chrome.storage.local.get(["ShowMediaPanel"],function(d){-1==d.ShowMediaPanel&&(c.F=!1)});this.i=this.G=null;this.D=!1;this.L()}var V=U.prototype;V.M=function(){var a=(this.v=!this.v)?"":"Off";chrome.action.setTitle({title:this.v?"Neat DL+":"Neat DL+ is Off\r\nClick to toggle"});chrome.action.setBadgeText({text:a})};V.Y=function(a){var b=this.h[[a.tabId,a.frameId]];b&&b["2"]!=a.url&&(b.postMessage([11,a.url]),b["2"]=a.url)};
...
V.I=async function(a){var _iPageUrl=a.pageUrl||a["5"]||"";var _iFileName=a["3"]||"";if(!_iFileName&&a["2"]){try{_iFileName=new URL(a["2"]).pathname.split("/").pop()}catch(_e){}}var _iSize=a["7"]||0;if(ExclusionManager.isWebsiteExcludedSync(_iPageUrl)||ExclusionManager.isFileTypeExcludedSync(_iFileName)||(a["7"]&&ExclusionManager.isFileSizeExcludedSync(_iSize))){this.i=null;return}if(this.D){var b="1:"+a["1"]+"\r\n";b+="2:"+a["2"]+"\r\n";a["3"]&&(b+="3:"+a["3"]+"\r\n");b+="6:"+(a["6"]||"normal")+"\r\n";a["4"]&&(b+="4:"+a["4"]+"\r\n");if(a.pageUrl){var c=a.pageUrl,d="";c&&=c.trim();c&&(d=(new URL(c)).origin);b+="Origin: "+d+"\r\n"}if(a.pageUrl){if(c=a.pageUrl)d=c.lastIndexOf("#"),c=0>d||d<c.indexOf("?")?c:c.substr(0,d);b+="Referer: "+c+"\r\n"}a["5"]&&(b+="5:"+a["5"]+"\r\n");a.cookies&&(b+="Cookie: "+a.cookies+"\r\n");a["10"]&&(b+="Content-Type: "+a["10"]+
...
60)&&parseInt(d%60)+" sec",l={6:"hls",fEx:"ts",4:"TS File "+b,fDu:b},l=M(l,{1:a["1"],2:a["2"],tabId:a.tabId,frameId:a.frameId,fS:a["7"],fileName:a.fileName}),Y(a,l),"POST"==l["1"]&&S(a,l),setTimeout(function(){f.h.A(l)},2500))}};V.A=function(a){var b=this.h[[a.tabId,a.frameId]];if(!b&&(b=this.h[[a.tabId,0]],!b))return;var _aTabUrl=(this.h[[a.tabId,0]]&&this.h[[a.tabId,0]]["2"])||"";if(ExclusionManager.isWebsiteExcludedSync(_aTabUrl))return;var _aFExt=a.fEx||"";if(_aFExt&&ExclusionManager.isFileTypeExcludedSync(_aFExt))return;var _aSize=a["7"]||a.fS||0;if(_aSize&&ExclusionManager.isFileSizeExcludedSync(_aSize))return;var c=a["2"],d=0,e;var f=0;for(e=c.length;f<e;f++){var g=c.charCodeAt(f);d=(d<<5)-d+g;d|=0}a.id=d;b.postMessage([1,a,b["2"]])};V.N=function(a){delete this.m[a.requestId]};
...
b.g&&256E4>=b["7"])&&"DCLK-AdSvr"!=L(b.B,"Server")){var J={2:b["2"],6:"media",1:b["1"],tabId:b.tabId,frameId:b.frameId,fEx:B.test(b.g)?b.g:b.s,7:b["7"],8:b["8"],fS:b["7"],fileName:b.fileName};if(!ExclusionManager.isFileSizeExcludedSync(b["7"])){"POST"==J["1"]&&S(b,J);Y(b,J);setTimeout(function(){d.A(J)},2E3)}}}else if(h||!this.v)this.u="";else{var _xPageUrl=(d.h[[b.tabId,0]]&&d.h[[b.tabId,0]]["2"])||"";if(ExclusionManager.isWebsiteExcludedSync(_xPageUrl)||ExclusionManager.isFileTypeExcludedSync(b.g||b.s||"")||ExclusionManager.isFileSizeExcludedSync(b["7"])){this.u=""}else{this.u=b["2"];g=d.h[[b.tabId,b.frameId]];p=d.h[[b.tabId,0]];var v=M(new T,{2:b["2"],1:b["1"],4:p&&p["4"]||g&&g["4"],5:p&&p["2"]||g&&g["2"],7:b["7"],8:b["8"],pageUrl:g&&g["2"]||b["2"]});chrome.tabs.query({active:!0,
...
c.postData);c["10"]&&(e["10"]=c["10"]);c["11"]&&(e["11"]=c["11"]);for(d in c)N(d.toLowerCase(),"x-")&&(e[d]=c[d]);this.i=e;chrome.cookies.getAll({url:e["2"]},this.J)}};V.$=function(a){try{void chrome.runtime.lastError}catch(_e){}for(var b in this.h)this.h[b]==a&&delete this.h[b];delete this.H[a.id]};V.la=function(a,b){var c=this.h;a=a.toString()+",";for(var d in c)N(d,a)&&function(){try{c[d].postMessage(b)}catch(_e){}}()};V.ga=function(a){var b=this.h,c;for(c in b)try{b[c].postMessage(a)}catch(_e){}};var _neatInstance=new U;

// --- Neat DL+ Message Handler (for popup & options page) ---
  ...
  switch (msg.action) {
    case 'getStatus':
      chrome.tabs.query({ active: true, currentWindow: true }, function(tabs) {
        ...
        sendResponse({
          enabled: _neatInstance.v,
          currentSiteExcluded: ExclusionManager.isWebsiteExcludedSync(currentUrl),
    ...
    case 'toggle':
      _neatInstance.M();
      sendResponse({ enabled: _neatInstance.v });
      break;

    case 'toggleSiteExclusion':
      (async function() {
        var hostname = msg.hostname || '';
        if (!hostname) { sendResponse({ excluded: false }); return; }
        var isExcluded = ExclusionManager.isWebsiteExcludedSync('https://' + hostname);
        if (isExcluded) {
          await ExclusionManager.removeExcludedWebsite(hostname);
          sendResponse({ excluded: false });
        } else {
          await ExclusionManager.addExcludedWebsite(hostname);
          sendResponse({ excluded: true });
        }
      })();
      return true;

    case 'getExcludedWebsites':
      ExclusionManager.getExcludedWebsites().then(function(list) {
        sendResponse({ websites: list });
      });
      return true;

    case 'addExcludedWebsite':
      ExclusionManager.addExcludedWebsite(msg.origin || msg.hostname || '').then(function(list) {
        sendResponse({ websites: list, success: true });
      });
      return true;

    case 'removeExcludedWebsite':
      ExclusionManager.removeExcludedWebsite(msg.origin || msg.hostname || '').then(function(list) {
        sendResponse({ websites: list, success: true });
      });
      return true;

    case 'getExcludedFileTypes':
      ExclusionManager.getExcludedFileTypes().then(function(list) {
        sendResponse({ fileTypes: list });
      });
      return true;

    case 'addExcludedFileType':
      ExclusionManager.addExcludedFileType(msg.pattern || '').then(function(list) {
        sendResponse({ fileTypes: list, success: true });
      });
      return true;

    case 'removeExcludedFileType':
      ExclusionManager.removeExcludedFileType(msg.pattern || '').then(function(list) {
        sendResponse({ fileTypes: list, success: true });
      });
      return true;

    case 'getSizeLimits':
      ExclusionManager.getSizeLimits().then(function(limits) {
        sendResponse({ sizeLimits: limits });
      });
      return true;

    case 'setSizeLimits':
      ExclusionManager.setSizeLimits(msg.sizeLimits || {}).then(function(limits) {
        sendResponse({ sizeLimits: limits, success: true });
      });
      return true;
  }
});
